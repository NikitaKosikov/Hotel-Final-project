...
<fmt:message bundle="${Loc}" key="Locale.link.blocked.services" var="blocked_services_link"/>
        ...
        <div style="font-size: 35px;" class="center colour">
            <c:if test="${sessionScope.user.roleId == 2}">
                <a class="nav-link a_not_decoration"
                   href="/Controller?command=gotoblockedservicespage">${blocked_services_link}
                </a>
                <a href="/Controller?command=gotoaddservicepage"
                   class="nav-link a_not_decoration">${add_service_link}
                </a>
            </c:if>
            ${services_name}
            <c:if test="${sessionScope.user.roleId == 2}">
                <div align="left">
                    ...
                    <div align="center" style="color: red; font-size: 17px">
                        <c:out value="${exist_service}"/>
                        <c:out value="${deleted_service_error}"/>
                        <c:out value="${change_service_error}"/>
                    </div>
                    <div align="center" class="error_place" style="top: 210px; color: #0fc60f; font-size: 17px">
                        <c:out value="${added_service}"/>
                        <c:out value="${deleted_service}"/>
                                    ...
                                    <div class="img_room" style="margin-right: 30px; margin-left: 57px;">
                                        <div>
                                            <div>
                                                ...
                                                <div>
                                                    <form action="/Controller?command=reservationservice" method="post">
                                                            ${date_service_name}
                                                        <input class="date_color" maxlength="10" type="text"
                                                               name="date_service"
                                                               placeholder="${format_date_placeholder}">
                                                        <input type="hidden" name="service_id"
                                                               value="${additional_services[status.begin + count].serviceId}">
                                                        <div style="margin-top: 15px">
                                                            <input type="submit" value="${order_name}">
                                                        </div>
                                                        <c:if test="${param.order_id !=null}">
                                                    ...
                                                    <c:if test="${sessionScope.user.roleId == 2}">
                                                        ...
                                                        <form class="rightstr" action="/Controller?command=blockservice"
                                                              method="post">
                                                            <input type="hidden" name="service_id"
                                                                   value="${additional_services[status.begin+count].serviceId}">
                                                            <input type="submit" value="${delete_service_button}">
                                                        </form>
                                                    </c:if>
                                                </div>
                                            </div>
