...
public class ChangeOrder implements Command {
    private static final Logger logger = Logger.getLogger(ChangeOrder.class.getName());
    ...
    private static final String AUTHORIZATION_ERROR_MESSAGE = "You must register";
    ...
    private static final String LOCALE_INVALID_DATE = "Locale.error.invalid.format.date";
    private static final String ACCESS_ROOM = "access_room";
    private static final String LOCALE_ACCESS_ROOM_MESSAGE = "Locale.unavailable.room";
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        logger.info("User wants to change order");
        ...
        try {
            ...
            response.sendRedirect(REDIRECT_TO_ACTIVE_ORDERS_PAGE + SEPARATE_PARAMETERS +
                    ERROR_DATE + EQUALLY_BETWEEN_PARAMETER_AND_VALUE + LOCALE_INVALID_DATE);
        ...
        try {
            if (orderService.changeOrder(resDate, orderId)){
                logger.info("Order successfully changed");
                response.sendRedirect(REDIRECT_TO_ACTIVE_ORDERS_PAGE);
            }else {
                logger.info("Order not changed");
                response.sendRedirect(REDIRECT_TO_ACTIVE_ORDERS_PAGE + SEPARATE_PARAMETERS +
                        ACCESS_ROOM + EQUALLY_BETWEEN_PARAMETER_AND_VALUE + LOCALE_ACCESS_ROOM_MESSAGE);
            }
        } catch (ServiceException e) {

            logger.log(Level.FATAL, "A server error occurred while changing the order", e);
            ...
            response.sendRedirect(REDIRECT_TO_ACTIVE_ORDERS_PAGE + SEPARATE_PARAMETERS +
                    ERROR_DATE + EQUALLY_BETWEEN_PARAMETER_AND_VALUE + e.getMessage());
        }
