...
public class GoToReturnedOrdersPage implements Command {
    private static final Logger logger = Logger.getLogger(GoToReturnedOrdersPage.class.getName());

    private static final String ROOMS = "rooms";
    private static final String ORDERS = "orderRooms";
    private static final String STATUS_ORDER = "отменен";
    ...
    private static final String FORWARD_TO_RETURNED_ORDERS_PAGE = "/WEB-INF/jsp/returned_orders.jsp";
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (user==null){
            request.setAttribute(AUTHORIZATION_ERROR, AUTHORIZATION_ERROR_MESSAGE);
            response.sendRedirect(REDIRECT_TO_AUTHORIZATION_PAGE);
            return;
        }

        logger.info("User wants to view the returned orders");
        ...
        try {
            ...
            if (!orderRooms.isEmpty()){
                ...
                logger.info("Returned orders found");
                List<Room> rooms = roomService.findRoomsByOrders(orderRooms);
                request.setAttribute(ROOMS, rooms);
            }else {
                logger.info("Returned orders not found");
                request.setAttribute(EMPTY_ORDER_LIST, EMPTY_ORDER_LIST_MESSAGE);
            }

            RequestDispatcher requestDispatcher = request.getRequestDispatcher(FORWARD_TO_RETURNED_ORDERS_PAGE);
            requestDispatcher.forward(request, response);
        } catch (ServiceException e) {

            logger.log(Level.FATAL, "A server error occurred while searching for returned orderRooms", e);
